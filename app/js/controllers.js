...
angular.module('myApp.controllers', ['kendo.directives'])
    .controller('HomeCtrl', ['$log', '$scope', '$timeout', 'careersService', 'd3Scales', 'localStorage', 'mergeSort', 'savedCareers', function($log, $scope, $timeout, careersService, d3Scales, localStorage, mergeSort, savedCareers) {
        ...
        $scope.currJobOptions = {
            autoBind: false,
            dataTextField: "career_name",
            dataValueField: "career_name",
            dataSource: dataSource,
            placeholder: 'Find the career that most closely matches your current one.',
            filter: 'contains'
        };
        ...
        //get career data
        $scope.getCareerEdLevels = function() {
            careersService.getCareerEdLevels().success(function(data) {
                $scope.careerEdLevels = data;

                //remove any undefined or null values
                for (var i = 0; i < $scope.careerEdLevels.length; i++) {
                    console.log($scope.careerEdLevels[i])
                    if(!$scope.careerEdLevels[i].career_edu) {
                        $scope.careerEdLevels.splice(i,1);
                    };
                }
            }).error(function(err) {
                $log.error('getCareerEdLevels error: ', err);
            });
        };
